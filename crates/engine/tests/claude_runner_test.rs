...
fn runs_and_captures_last_line_as_artifact() {
    ...
    let out = r
        .run("实现功能", None, false, None, None, &PathBuf::from("."))
        .expect("ok");
...
fn skill_prefixes_prompt() {
    let r = ClaudeRunner::new(fixture("stub-claude.sh"));
    let out = r
        .run("审查", Some("four-dimension-review"), false, None, None, &PathBuf::from("."))
        .unwrap();
...
fn times_out_on_hanging_cli() {
    ...
    let res = r.run("x", None, false, Some(1), None, &PathBuf::from("."));
