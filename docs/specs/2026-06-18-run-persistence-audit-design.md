...
CLI 的 `for event in erx` 循环里,收到 `RunStarted` 时 `RunRecorder::open(...)` **并立刻 `record(&RunStarted)`**(open 只建文件,RunStarted 也要作为第一行落盘,否则"首行=RunStarted"约定落空);之后每个事件先 `recorder.record(&event)` 再走现有渲染 —— 在现有 match 之前 tee 一行,**不改任何现有渲染分支**。

> 设计信条:
> - **审计是旁路**:`record` 内部 I/O 错误只 `eprintln!` 告警,绝不冒泡打断 run(对齐全局基线"日志/审计失败 fail-safe,不破坏主路径")。
> - **每条 flush**:`record` 写完一行立即 flush(`BufWriter` 默认缓冲会在进程崩溃时丢掉最后几行 —— 而崩溃前那几条恰恰是排错最需要的)。审计文件吞吐不是瓶颈,可靠性优先。
...
- **stdout/stderr 分离**:`--json` 下,机器数据走 stdout(**与落盘同一 `{ts, event}` 行 schema**,见 §5,保证 `view`/管道/文件三处格式一致),所有人读提示/进度走 stderr(ccswarm 同款,便于 `agentpipe run x.yaml --json | jq`)。非 `--json` 维持现状(人读走 stdout)。
- 退出码:run 成功 0 / 失败非 0;validate 同理 —— 便于脚本与未来 CI 串接。

### 6.1 渲染必须先与交互解耦(前置重构)

现有 `cli/main.rs` 的事件循环把**展示**和**交互**焊在一起:遇到 `StepAwaitingGate` 直接 `prompt_gate(...)` 阻塞读 stdin(main.rs:95/117)。但 `view`(重读已完成 run 的 NDJSON)和 `--dry-run` 的日志里也会出现 `StepAwaitingGate` —— 若照搬同一循环,会在重放历史时**卡住等用户输入**,这是个真 bug。

落地前必须先抽出纯函数:

```rust
fn render_event(event: &Event) -> String   // 事件 → 人读一行,无任何 I/O / stdin
```

- `run`(交互):`render_event` 打印 + 对 `StepAwaitingGate` 走 `prompt_gate` 收命令(现状行为)。
- `view` / `--dry-run`(只读):**只调 `render_event`**,对 `StepAwaitingGate` 仅渲染"⏸ 当时在此等待",绝不读 stdin。

这是 §6 所有只读子命令的前提,排在 plan 第一步(见 §11)。
...
- engine:加 `chrono` 直接依赖(run-id 时间戳 + 行 ts)。
...
1. 抽 `render_event(&Event) -> String` 纯函数,`run` 改用它 + 保留交互 gate(§6.1,前置重构,行为不变)。
2. engine `audit` 模块 + `RunRecorder`(每条 flush)+ run-id/slug/allowlist + 单测。
3. cli 迁 clap,接 `RunRecorder` 进事件循环(run 落盘,首行 RunStarted),`--json` 分流(同 `{ts,event}` schema)。
4. `validate` / `--dry-run`(纯前置,不依赖 NDJSON,可与 2 并行;dry-run 用 §6.1 只读渲染)。
5. `view` / `cost` / `runs`(读 NDJSON,只读渲染)。
6. `diff`(依赖 5 的读取层)。
