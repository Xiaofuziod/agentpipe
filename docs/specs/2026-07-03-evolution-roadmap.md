...
- A1 是 B 的硬前置：无人值守下预算是第一道安全阀。注意 A1 的机制是把洞**显式化**（校验硬拦 + allow_unmetered 签字）而非物理堵上（metrics 接通前 ACP 花费仍不可见）；watch 侧因此对 allow_unmetered 模板默认拒绝、需二次签字（见 watch spec §4.1），两层配合才构成无人值守的预算安全。
